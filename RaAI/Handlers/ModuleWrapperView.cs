...
{ 
    // Adapter around your existing ModuleWrapper (or whatever object lives in moduleManager.Modules)
    // Provides a stable view for the UI without changing existing framework types.
    public class ModuleWrapperView { private readonly object inner; private readonly Type innerType;
    ...
    // Name: try properties "Name" or "Instance.Name"
    public string Name
    {
        get
        {
    ...
    // Enabled: try property on wrapper or fallback true. Uses safe conversion to avoid int->bool reflection errors.
    public bool Enabled
    {
        get
        {
    ...
    // TimeoutMs: if not present, default to 2000
    ...
    // Logs: try property "Logs" -> IList<string> fallback empty list
    ...
    // Process: prefer async method ProcessAsync(string) -> Task<string>, else fallback to Process(string)
    public async Task<string> ProcessAsync(string input)
    {
        // try wrapper.ProcessAsync
        ...
        // fallback: wrapper.Process(string) or instance.Process
    ...
    // Try to get inner Instance property (common pattern)
    ...
    // Expose the raw inner object for advanced usage
