...
namespace ASHATCore.Modules.Speech;

[RaModule(Category = "core")]
public partial class SpeechModule : ModuleBase, IDisposable
{
    ...
    public override string Process(string input)
    {
        try
        {
            // Synchronously wait for the async result
            var result = GenerateResponseAsync(input, "", "", CancellationToken.None).GetAwaiter().GetResult();
    ...
    public async Task<string> GenerateResponseAsync(string input, string input1, string v, CancellationToken cancellationToken = default)
    {
        ...
        if (mDo.Success)
        {
            var utteASHATnce = mDo.Groups[1].Value.Trim();
            return await HandleAgenticCommandAsync(utteASHATnce, cancellationToken);
        }
    ...
    // ---- Events ----
    private void OnMemoryReady(ASHATCore.Engine.Memory.MemoryModule memory)
    {
        _memoryInst = memory;
    ...
    public override void OnSystemEvent(string name, object? payload)
    {
        ...
        {
            if (payload is ASHATCore.Engine.Memory.MemoryModule mm) OnMemoryReady(mm);
