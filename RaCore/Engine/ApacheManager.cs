...
public class ApacheManager
{
    ...
    private static readonly string StaticConfigPath = @"C:\RaOS\webserver\settings";
    ...
    public static (bool valid, List<string> issues, string? message) VerifyApacheConfig(string configPath)
    {
        ...
        try
        {
            ...
            // Check if RaCore configuration marker exists
            if (!config.Contains("# RaCore Configuration"))
            {
                issues.Add("RaCore configuration not found");
            }
    ...
    public static bool ConfigurePhpIni(string phpIniPath)
    {
        try
        {
            ...
            var backupPath = phpIniPath + $".racore_backup_{DateTime.UtcNow:yyyyMMddHHmmss}";
