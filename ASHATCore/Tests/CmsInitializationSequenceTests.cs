...
public class CmsInitializationSequenceTests
{
    ...
    private static void TestInitializationOrder()
    {
        ...
        var testDir = Path.Combine(Path.GetTempPath(), "ASHATCore_init_order_test_" + Guid.NewGuid().ToString());
        ...
        try
        {
            ...
            // module to be present, which would require full module loading.
    ...
    private static void TestFirstRunMarkerCreation()
    {
        ...
        // Create a temporary directory for testing
        var testDir = Path.Combine(Path.GetTempPath(), "ASHATCore_marker_test_" + Guid.NewGuid().ToString());
        Directory.CreateDirectory(testDir);
