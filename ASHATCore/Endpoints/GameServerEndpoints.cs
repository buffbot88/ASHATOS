...
public static class GameServerEndpoints
{
        ...
        IAuthenticationModule? authModule)
    {
        ...
        {
            Console.WriteLine("[ASHATCore] GameServer module or Authentication not available - skipping GameServer endpoints");
            return app;
        }

        // Helper function to validate auth and permissions
        async Task<(bool authorized, User? user, string? error)> ValidateGameServeASHATccess(HttpContext context, UserRole requiredRole = UserRole.User)
        {
        ...
        app.MapPost("/api/gameserver/create", async (HttpContext context) =>
        {
            try
            {
                var (authorized, user, error) = await ValidateGameServeASHATccess(context, UserRole.User);
                if (!authorized)
                {
        ...
        app.MapGet("/api/gameserver/games", async (HttpContext context) =>
        {
            try
            {
                var (authorized, user, error) = await ValidateGameServeASHATccess(context);
                if (!authorized)
                {
                    context.Response.StatusCode = error == "No token provided" || error == "Invalid or expired token" ? 401 : 403;
                    await context.Response.WriteAsJsonAsync(new { success = false, message = error });
        ...
        app.MapGet("/api/gameserver/game/{gameId}", async (HttpContext context) =>
        {
            try
            {
                var (authorized, _, error) = await ValidateGameServeASHATccess(context);
                if (!authorized)
                {
                    context.Response.StatusCode = error == "No token provided" || error == "Invalid or expired token" ? 401 : 403;
                    await context.Response.WriteAsJsonAsync(new { success = false, message = error });
        ...
        app.MapPost("/api/gameserver/game/{gameId}/deploy", async (HttpContext context) =>
        {
            try
            {
                var (authorized, user, error) = await ValidateGameServeASHATccess(context, UserRole.Admin);
                if (!authorized)
                {
                    context.Response.StatusCode = error == "No token provided" || error == "Invalid or expired token" ? 401 : 403;
                    await context.Response.WriteAsJsonAsync(new { success = false, message = error });
        ...
        Console.WriteLine("[ASHATCore] GameServer API endpoints registered:");
