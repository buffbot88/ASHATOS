...
public class ApacheManager
{
    ...
    public bool ConfigureReverseProxy(int racorePort = 5000, string domain = "localhost")
    {
        ...
        try
        {
            ...
            // Check if proxy modules are already enabled (not commented out)
            var proxyModuleEnabled = System.Text.RegularExpressions.Regex.IsMatch(
                config, @"^\s*LoadModule\s+proxy_module\s+modules/mod_proxy\.so", 
                System.Text.RegularExpressions.RegexOptions.Multiline);
            var proxyHttpModuleEnabled = System.Text.RegularExpressions.Regex.IsMatch(
...
";
            ...
            else
            {
                ...
                Console.WriteLine("[ApacheManager] ℹ️  RaCore reverse proxy configuration found, verifying...");
                
                var needsUpdate = false;
                var updateReasons = new List<string>();
                
                // Check if ServerAlias includes required domains
                if (!config.Contains("ServerAlias") || 
                    !(config.Contains("agpstudios.online") && config.Contains("www.agpstudios.online")))
                {
                    needsUpdate = true;
                    updateReasons.Add("Missing or incomplete ServerAlias configuration");
                }
                
                // Check if correct port is configured
                var currentPortPattern = @"http://localhost:(\d+)/";
                ...
                // Check if WebSocket support is configured
                if (!config.Contains("ProxyPass /ws ws://"))
                {
                    needsUpdate = true;
                    updateReasons.Add("Missing WebSocket support");
                }
                
                if (needsUpdate)
                {
                    Console.WriteLine("[ApacheManager] ⚠️  Configuration is incomplete or outdated:");
                    foreach (var reason in updateReasons)
                    {
                        Console.WriteLine($"[ApacheManager]    - {reason}");
                    }
                    Console.WriteLine("[ApacheManager] 🔄 Updating RaCore configuration...");
                    
                    // Remove old RaCore configuration
                    var startMarker = "# RaCore Reverse Proxy Configuration";
                    var startIndex = config.IndexOf(startMarker);
                    if (startIndex >= 0)
                    {
                        // Find the end of the VirtualHost block
                        var endMarker = "</VirtualHost>";
                        var endIndex = config.IndexOf(endMarker, startIndex);
                        if (endIndex >= 0)
                        {
...
";
                    config += proxyConfig;
                    Console.WriteLine($"[ApacheManager] ✅ Updated reverse proxy configuration for {domain}:{racorePort}");
                    Console.WriteLine($"[ApacheManager] ✅ ServerAlias configured for: agpstudios.online, www.agpstudios.online");
                    modified = true;
                }
                else
                {
                    Console.WriteLine("[ApacheManager] ✅ Configuration is up-to-date");
                }
            }
...
";
    ...
    public static bool ConfigurePhpIni(string? phpIniPath = null)
    {
        ...
        if (phpIniPath == null)
        {
            Console.WriteLine("[ApacheManager] ⚠️  Could not find php.ini");
            Console.WriteLine("[ApacheManager] PHP configuration will use defaults");
            return false;
        }
        
        try
        {
            Console.WriteLine($"[ApacheManager] Found PHP config: {phpIniPath}");
            
            // Read existing config
            var config = File.ReadAllText(phpIniPath);
            var modified = false;
            
            // Define required settings with their recommended values
            ...
            foreach (var setting in requiredSettings)
            {
                ...
                var pattern = $@"^\s*;?\s*{System.Text.RegularExpressions.Regex.Escape(key)}\s*=.*$";
                var match = System.Text.RegularExpressions.Regex.Match(
                    config, pattern, System.Text.RegularExpressions.RegexOptions.Multiline);
                
                if (match.Success)
                {
                    // Check if current value is different
                    var currentLine = match.Value;
                    var expectedLine = $"{key} = {value}";
                    
                    if (!currentLine.Contains(value) || currentLine.TrimStart().StartsWith(";"))
                    {
                        // Update the setting
                        config = System.Text.RegularExpressions.Regex.Replace(
                            config, pattern, expectedLine, 
                            System.Text.RegularExpressions.RegexOptions.Multiline);
                ...
                {
                    // Setting not found, add it to the appropriate section
                    var section = key.Contains(".") ? $"[{key.Split('.')[0]}]" : "[PHP]";
                    
                    if (config.Contains(section))
                    {
                        // Add after the section header
                        var sectionIndex = config.IndexOf(section);
                        var nextLineIndex = config.IndexOf('\n', sectionIndex) + 1;
                        config = config.Insert(nextLineIndex, $"{key} = {value}\n");
                        updates.Add($"{key} = {value} (added)");
                        modified = true;
                    }
                    else
                    {
                        // Add section and setting at the end
                        config += $"\n{section}\n{key} = {value}\n";
                        updates.Add($"{key} = {value} (new section)");
                        modified = true;
                    }
                }
            }
            ...
            {
                // Create backup
                var backupPath = phpIniPath + ".racore_backup_" + DateTime.Now.ToString("yyyyMMddHHmmss");
                File.Copy(phpIniPath, backupPath);
                Console.WriteLine($"[ApacheManager] 💾 Backup created: {backupPath}");
                
                // Write modified config
                File.WriteAllText(phpIniPath, config);
                Console.WriteLine($"[ApacheManager] ✅ PHP configuration updated: {phpIniPath}");
                Console.WriteLine($"[ApacheManager] Updated settings:");
                foreach (var update in updates)
                {
                    Console.WriteLine($"[ApacheManager]    - {update}");
                }
                return true;
            }
            else
            {
                Console.WriteLine("[ApacheManager] ✅ PHP configuration is already optimal");
                return true;
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ApacheManager] ⚠️  Error updating PHP config: {ex.Message}");
            return false;
        }
    }
}
