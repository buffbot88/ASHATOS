...
/// Provides protection, guidance, and narrative support for players
/// </summary>
public sealed class GuardianAngelService
{
    ...
    public PlayerGuardianState InitializeGuardian(string playerId)
    {
        var state = new PlayerGuardianState
        {
            ...
            NarrativeMode = NarrativeMode.Interactive
        };
    ...
    public GuardianGuidance ProvideGuidance(string playerId, PlayerSituation situation)
    {
        ...
        var guidance = new GuardianGuidance
        {
            ...
            NarrativeHint = GenerateNarrativeHint(situation)
        };
    ...
    /// <summary>
    /// Generate narrative interaction for immersive Guardian Angel experience
    /// </summary>
    public NarrativeInteraction GenerateNarrative(string playerId, GameContext context)
    {
        if (!_playerStates.TryGetValue(playerId, out var state))
        {
            state = InitializeGuardian(playerId);
        ...
        var narrative = new NarrativeInteraction
        {
            PlayerId = playerId,
            NarratorName = "ASHAT",
        ...
        state.LastNarrativeAt = DateTime.UtcNow;
    ...
    public void UpdateGuardianSettings(string playerId, ProtectionLevel? protection = null, 
        GuidanceStyle? guidance = null, NarrativeMode? narrative = null)
    {
        if (!_playerStates.TryGetValue(playerId, out var state))
        {
            state = InitializeGuardian(playerId);
        ...
        if (narrative.HasValue)
            state.NarrativeMode = narrative.Value;
    ...
    private string GenerateNarrativeHint(PlayerSituation situation)
    {
    ...
    private string GenerateContextualDialogue(GameContext context, PlayerGuardianState state)
    {
        ...
        if (context.IsFirstMeeting && state.NarrativeCount == 0)
        {
    ...
    private InteractionType DetermineInteractionType(GameContext context)
    {
        ...
        if (context.MajorAchievement)
            return InteractionType.Celebration;
        if (context.PlayerStruggling)
...
public class PlayerGuardianState
{
    ...
    public DateTime? LastNarrativeAt { get; set; }
}
...
public class GuardianGuidance
{
    ...
    public string NarrativeHint { get; set; } = string.Empty;
}
...
public class NarrativeInteraction
{
    public string PlayerId { get; set; } = string.Empty;
