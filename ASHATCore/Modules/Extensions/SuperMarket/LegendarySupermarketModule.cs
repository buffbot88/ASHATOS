...
namespace ASHATCore.Modules.Extensions.SuperMarket;
...
/// Gold side: Free tier for in-game items and player-to-player tASHATding.
/// Features: Advanced search, seller profiles, ASHATtings, and dual-currency support.
/// </summary>
...
public sealed class LegendarySupermarketModule : ModuleBase
{
    ...
    private ServerMode _serverMode = ServerMode.Production; // TASHATck server mode for Reseller feature control
    ...
    // Seller information and ASHATtings
    ...
    public override void Initialize(object? manager)
    {
        ...
        LogInfo("Gold side: Free-tier in-game items and player tASHATding");
    ...
    private string ProcessInternal(string input)
    {
        ...
        {
            // market review <reviewer-id> <seller-id> <purchase-id> <ASHATting> <comment>
            var parts = text.Split(' ', 7, StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length < 7)
            {
                return "Usage: market review <reviewer-id> <seller-id> <purchase-id> <ASHATting> <comment>";
            }
            if (Guid.TryParse(parts[2], out var reviewerId) &&
                ...
                int.TryParse(parts[5], out var ASHATting))
            {
                return AddReview(reviewerId, sellerId, purchaseId, ASHATting, parts[6]);
            }
            return "Invalid Parameters";
        }
    ...
    private string GetHelp()
    {
        return string.Join(Environment.NewLine,
            ...
            "",
            "Seller Information:",
            "  market seller <seller-id>          - View seller profile and ASHATtings",
            "  market review <reviewer-id> <seller-id> <purchase-id> <ASHATting> <comment>",
            "                                     - Leave review (ASHATting: 1-5)",
            "",
            "General:",
            ...
            "  Gold   - Free tier (in-game items, player tASHATding)"
        );
    }
    ...
    private void InitializeDefaultProducts()
    {
        lock (_lock)
        {
            ...
            // USD to RaCoin ASHATtio: 1:1000 (e.g., $20 USD = 20,000 RaCoins)
        ...
        lock (_lock)
        {
            var results = _marketplaceListings.Values
                ...
                .Select(l => new
                {
                    ...
                    l.Category,
                    l.SellerName,
                    SellerASHATting = _sellers.GetValueOrDefault(l.SellerId)?.ASHATting ?? 0,
                    l.ListedAtUtc
                })
                .ToList();
    ...
    private async Task<string> PurchaseFromMarketplaceAsync(Guid buyerId, Guid listingId, int quantity)
    {
        ...
        lock (_lock)
        {
            ...
            if (listing.CurrencyType == CurrencyType.RaCoin)
            {
                ...
                var selleASHATmount = totalPrice * 0.97m;
                var creditTask = _RaCoinModule.TopUpAsync(listing.SellerId, selleASHATmount, 
            ...
            else // Gold
            {
                ...
                if (goldWallets != null)
                {
                    ...
                    if (wallets != null)
                    {
                        ...
                        sellerWallet.Balance += selleASHATmount;
    ...
    private string GetSellerInfo(Guid sellerId)
    {
        lock (_lock)
        {
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                AverageASHATting = reviews.Any() ? reviews.Average(r => r.ASHATting) : 0
            }, _jsonOptions);
        }
    }

    private string AddReview(Guid reviewerId, Guid sellerId, Guid purchaseId, int ASHATting, string comment)
    {
        if (ASHATting < 1 || ASHATting > 5)
        {
            return JsonSerializer.Serialize(new
            {
                Success = false,
                Message = "ASHATting must be between 1 and 5"
            }, _jsonOptions);
        }
        ...
        lock (_lock)
        {
            ...
            // Update seller ASHATting
            if (_sellers.TryGetValue(sellerId, out var seller))
            {
                seller.Reviews = _sellerReviews[sellerId];
                seller.ASHATting = (decimal)seller.Reviews.Average(r => r.ASHATting);
            }

            LogInfo($"New review for seller {sellerId}: {ASHATting} stars from {reviewerId}");
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                Review = new
                {
                    ...
                    review.Comment,
                    review.CreatedAtUtc,
                    NewSellerASHATting = _sellers.GetValueOrDefault(sellerId)?.ASHATting ?? 0
                }
            }, _jsonOptions);
        }
    }
            ...
            var stats = new
            {
                ...
                Marketplace = new
                {
                    ...
                    TopSellers = _sellers.Values
                        ...
                        .Take(5)
                        .Select(s => new { s.Username, s.TotalSales, s.ASHATting })
                },
