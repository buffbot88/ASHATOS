...
namespace ASHATCore.Modules.Extensions.LegendaryPay;

/// <summary>
/// LegendaryPay Module - Next-Generation payment system for ASHATOS and Generated content platforms.
...
public sealed class LegendaryPayModule : ModuleBase
{
    ...
    private readonly Dictionary<Guid, List<PaymentAction>> _useASHATctions = new();
    ...
    private string GetHelp()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("Next-Generation payment system for ASHATOS platforms");
    ...
    private string GetStatus()
    {
        lock (_lock)
        {
            return JsonSerializer.Serialize(new
            {
                ...
                {
                    Enabled = _isDevMode,
                    RewardPeASHATction = $"{_devModeRewardAmount} Gold",
                    AppliesTo = new[]
                    {
                ...
                Statistics = new
                {
                    ...
                    TotalUsers = _useASHATctions.Count
                },
    ...
    private string ProcessPaymentAction(Guid userId, string description)
    {
        lock (_lock)
        {
            ...
            if (!_useASHATctions.ContainsKey(userId))
            {
                _useASHATctions[userId] = new List<PaymentAction>();
            }
            _useASHATctions[userId].Add(action);
    ...
    private string GetUserPaymentHistory(Guid userId)
    {
        lock (_lock)
        {
            if (!_useASHATctions.TryGetValue(userId, out var actions))
            {
                return JsonSerializer.Serialize(new
                {
                    UserId = userId,
        ...
        lock (_lock)
        {
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                Statistics = new
                {
                    ...
                    AverageRewardPeASHATction = _isDevMode && totalRewards > 0 
                        ? $"{totalRewards / _paymentActions.Values.Count(a => a.IsApproved)} Gold" 
                        : "N/A"
                },
                TopUsers = _useASHATctions
                    .Select(kvp => new
                    {
                        UserId = kvp.Key,
