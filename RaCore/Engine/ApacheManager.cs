...
public class ApacheManager
{
    ...
    /// <summary>
    /// Updates existing RaCore reverse proxy configuration with a new port
    /// </summary>
    /// <returns>True if successful, false otherwise</returns>
    public static bool UpdateRaCoreProxyPort(int newPort, string domain = "localhost")
    {
        var configPath = FindApacheConfigPath();
        if (configPath == null || !File.Exists(configPath))
        {
        ...
        try
        {
            ...
            // Check if RaCore proxy configuration exists
            if (!config.Contains("# RaCore Reverse Proxy Configuration"))
            {
                return false;
            }
            ...
            // Update ProxyPass and ProxyPassReverse directives with new port
            config = System.Text.RegularExpressions.Regex.Replace(
                config,
                @"ProxyPass\s+/\s+http://localhost:\d+/",
                $"ProxyPass / http://localhost:{newPort}/"
            );
            
            config = System.Text.RegularExpressions.Regex.Replace(
                config,
                @"ProxyPassReverse\s+/\s+http://localhost:\d+/",
                $"ProxyPassReverse / http://localhost:{newPort}/"
            );

            // Update WebSocket proxy directives
            config = System.Text.RegularExpressions.Regex.Replace(
                config,
                @"ProxyPass\s+/ws\s+ws://localhost:\d+/ws",
                $"ProxyPass /ws ws://localhost:{newPort}/ws"
            );
            
            config = System.Text.RegularExpressions.Regex.Replace(
                config,
                @"ProxyPassReverse\s+/ws\s+ws://localhost:\d+/ws",
                $"ProxyPassReverse /ws ws://localhost:{newPort}/ws"
            );

            // Update the auto-generated comment timestamp
            config = System.Text.RegularExpressions.Regex.Replace(
                config,
                @"# Auto-generated by RaCore ApacheManager on .+ UTC",
                $"# Auto-generated by RaCore ApacheManager on {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} UTC"
            );

            // Write updated config
            File.WriteAllText(configPath, config);
            Console.WriteLine($"[ApacheManager] ✅ Apache configuration updated to port {newPort}");
            Console.WriteLine($"[ApacheManager] ⚠️  Please restart Apache for changes to take effect!");

            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ApacheManager] ❌ Error updating Apache configuration: {ex.Message}");
            return false;
        }
    }
