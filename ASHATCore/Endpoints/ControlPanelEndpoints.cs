...
public static class ControlPanelEndpoints
{
    ...
    public static WebApplication MapControlPanelEndpoints(this WebApplication app,
        ...
        ASHATCore.Engine.FirstRunManager? firstRunManager = null)
    {
        if (authModule == null)
        {
            Console.WriteLine("[ASHATCore] Authentication module not available - skipping Control Panel endpoints");
            return app;
        }
...
app.MapPost("/api/market/review", async (HttpContext context) =>
{
    ...
    var command = $"market review {body.ReviewerId} {body.SellerId} {body.PurchaseId} {body.ASHATting} {body.Comment}";
    var result = marketModule.Process(command);
    return Results.Content(result, "application/json");
});
...
app.MapPost("/api/control/activate", async (HttpContext context) =>
{
    ...
    private static bool IsValidLicenseFormat(string licenseKey)
    {
        ...
        // Accept format: ASHATOS-XXXX-XXXX-XXXX-XXXX or similar patterns
        var parts = licenseKey.Split('-');
        return parts.Length >= 2 && parts[0].Equals("ASHATOS", StringComparison.OrdinalIgnoreCase);
    }
