...
public static class Phase937Tests
{
    ...
    private static void TestBotDetection()
    {
        ...
        foreach (var (userAgent, expectedIsBot, expectedBotName) in testCases)
        {
            ...
            {
                passed++;
                Console.WriteLine($"✅ PASS: {(string.IsNullOrEmpty(userAgent) ? "[empty]" : userAgent.Substring(0, Math.Min(50, userAgent.Length)))}...");
                if (isBot)
                {
            ...
            {
                failed++;
                Console.WriteLine($"❌ FAIL: {(string.IsNullOrEmpty(userAgent) ? "[empty]" : userAgent)}");
