...
namespace RaCore.Modules.Core.AssetSecurity;

/// <summary>
/// Asset Security Module for RaOS Phase 9.3.6
...
public sealed class AssetSecurityModule : ModuleBase, IAssetSecurityModule
{
    ...
    private async Task ScanBinaryDataForWatermarksAsync(byte[] data, WatermarkScanResult result)
    {
        ...
        var watermarkKeywords = new[] { "©", "copyright", "trademark", "™", "®" };
    ...
    {
        // Check name and description for known brand/company patterns
        var knownBrands = new[] { "unity", "unreal", "adobe", "autodesk", "blender", "epic", "valve" };
        
        foreach (var brand in knownBrands)
        {
            if (name.Contains(brand, StringComparison.OrdinalIgnoreCase) || 
                description.Contains(brand, StringComparison.OrdinalIgnoreCase))
            {
                result.DetectedWatermarks.Add(new WatermarkInfo
                {
                    Type = "trademark",
                    Content = $"Reference to '{brand}'",
