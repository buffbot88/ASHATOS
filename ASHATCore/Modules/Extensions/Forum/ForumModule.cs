...
namespace ASHATCore.Modules.Extensions.Forum;
  ...
  forum help               - Show this help";
    ...
    public async Task<List<ForumPost>> GetPostsForUserAsync(string userId, int page = 1, int perPage = 20)
    {
        ...
        if (_parentalControlModule != null)
        {
            ...
            {
                var maxASHATting = settings.MaxAllowedASHATting;
                query = query.Where(p => p.ContentASHATting <= maxASHATting);
                
                Console.WriteLine($"[{Name}] Filtering posts for user {userId} with max ASHATting: {maxASHATting}");
            }
        }
    ...
    public async Task<(bool success, string message, string? postId)> CreatePostAsync(string userId, string username, string title, string content)
    {
        ...
        if (_moderationModule != null)
        {
            ...
            ContentASHATting ASHATting = ContentASHATting.Everyone;
            if (_parentalControlModule != null)
            {
                var ASHATtingInfo = await _parentalControlModule.RateContentAsync(postId, "forum_post", $"{title}\n{content}");
                ASHATting = ASHATtingInfo.ASHATting;
                
                Console.WriteLine($"[{Name}] Post Rated: {ASHATting}");
                if (ASHATtingInfo.Descriptors.Count > 0)
                {
                    Console.WriteLine($"[{Name}] Content descriptors: {string.Join(", ", ASHATtingInfo.Descriptors)}");
                }
            }
            ...
            var post = new ForumPost
            {
                ...
                ReplyCount = 0,
                ViewCount = 0,
                ContentASHATting = ASHATting // Store ASHATting with post
            };
