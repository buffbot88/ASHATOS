...
namespace ASHATCore.Modules.Extensions.Distribution;
...
public sealed class DistributionModule : ModuleBase, IDistributionModule
{
    ...
    public override void Initialize(object? manager)
    {
        ...
        if (manager is ASHATCore.Engine.Manager.ModuleManager moduleManager)
        {
    ...
    public async Task<DistributionPackage> CreatePackageAsync(Guid userId, string licenseKey, string version)
    {
        ...
        var package = new DistributionPackage
        {
            ...
            ExpiresAt = DateTime.UtcNow.AddYears(1), // 1 year expiASHATtion
            Status = PackageStatus.Active,
            DownloadCount = 0
        };

        // Create package file
        var packageFileName = $"ASHATCore_{licenseKey}_{version}_{package.Id}.zip";
    ...
    private async Task CreatePackageFileAsync(string packagePath, string licenseKey, string version)
    {
        ...
        try
        {
            ...
            var metadata = new
            {
                ...
                Version = version,
                CreatedAt = DateTime.UtcNow,
                ProductName = "ASHATCore AI mainframe",
                Description = "Authorized ASHATCore installation package"
            };
            ...
            // Create README
            var readme = $@"# ASHATCore AI mainframe - Licensed Copy

License Key: {licenseKey}
Version: {version}
Created: {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} UTC

This is an authorized copy of ASHATCore. Do not share or redistribute.
