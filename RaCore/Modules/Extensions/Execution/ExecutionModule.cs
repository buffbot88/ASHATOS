using RaCore.Modules.Extensions.Skills;
...
namespace RaCore.Modules.Extensions.Execution;
...
public sealed class ExecutionModule : ModuleBase
{
    ...
    public async Task<ModuleResponse> ProcessAsync(string input)
    {
        ...
        foreach (var step in plan.Steps)
        {
            ...
            {
                var crashMsg = $"(step crashed) {step.Skill}: {ex.Message}";
                return await _thoughtProcessor.ProcessThoughtAsync(crashMsg, null, null, new System.Collections.Concurrent.ConcurrentQueue<Thought>(), Mood.Confused);
            }
