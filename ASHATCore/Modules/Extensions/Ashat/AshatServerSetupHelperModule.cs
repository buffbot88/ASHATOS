...
using ASHATCore.Modules.Extensions.ServerSetup;

namespace ASHATCore.Modules.Extensions.Ashat;

/// <summary>
/// ASHAT Server Setup Helper Module
/// Provides Interactive guidance for server administASHATtors setting up ASHATOS public servers
...
public sealed class AshatServerSetupHelperModule : ModuleBase
{
    ...
    private void InitializeTrainingCourses()
    {
        _availableCourses["ftp-basics"] = new CourseInfo
        {
            ...
            Description = "Introduction to FTP server Configuration for ASHATOS",
        ...
        _availableCourses["ftp-security"] = new CourseInfo
        {
            ...
            Description = "Advanced security pASHATctices for FTP access",
            Lessons = new List<string>
            {
        ...
        _availableCourses["public-server-launch"] = new CourseInfo
        {
            ...
            Description = "Step-by-step guide to launching a ASHATOS public server",
            Lessons = new List<string>
            {
                "Pre-launch server health check",
                "FTP setup and Configuration",
                "Admin instance creation",
                "Security best pASHATctices",
...
Examples:
  ...
  ashat setup launch";
    ...
    private string GetServerSetupGuide()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("Welcome! I'll guide you through setting up your ASHATOS public server with FTP access.");
        sb.AppendLine();
        sb.AppendLine("=== Prerequisites ===");
        sb.AppendLine("✓ Linux server (Ubuntu, Debian, CentOS, etc.)");
        sb.AppendLine("✓ Root/sudo access");
        sb.AppendLine("✓ ASHATCore installed and running");
        sb.AppendLine();
        ...
        sb.AppendLine("   Command: serversetup ftp createuser username=ASHATOS_ftp path=/path/to/ASHATOS");
        sb.AppendLine("   Purpose: Create secure, restricted FTP user for ASHATOS folder only");
        sb.AppendLine();
    ...
    private string GetFtpConfigureGuide()
    {
        ...
        sb.AppendLine("   - Username: Your Linux username (e.g., ASHATCore)");
    ...
    private string GetFtpSecureGuide()
    {
        ...
        sb.AppendLine("  ✓ User is restricted to ASHATOS folder only (chroot jail)");
        ...
        sb.AppendLine("  $ serversetup ftp createuser username=ASHATOS_ftp path=/path/to/ASHATOS");
        sb.AppendLine();
    ...
    private string GetRecommendationForIssue(string issue)
    {
        if (issue.Contains("Databases"))
            return "  → Check if ASHATCore is running in the correct directory\n" +
                   "  → Verify folder permissions: ls -la Databases/";
        
        if (issue.Contains("PHP"))
            return "  → Create PHP folder: mkdir php\n" +
                   "  → Verify folder permissions: sudo chown ASHATCore:ASHATCore php/";
        ...
        if (issue.Contains("FTP"))
            return "  → FTP folder will be created automatically\n" +
                   "  → Verify you have write permissions in ASHATCore directory";
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("✅ ASHATOS Public Server Setup Checklist");
        sb.AppendLine();
        sb.AppendLine("Prerequisites:");
        sb.AppendLine("  [ ] Linux server (Ubuntu, Debian, CentOS, etc.)");
        sb.AppendLine("  [ ] Root/sudo access");
    ...
    private string GetCourseContent(string courseId)
    {
        ...
        // Provide related pASHATctical commands
